...
options, remainder = gopt.getopt(sys.argv[1:], '', ["path_data=",
                                                    "n_samples=",
                                                    "n_iter="])
path_data = "../../data/natural_scenes/"
n_samples = -1
n_iter = 3                         ## total number passes through dataset
exp_dir = "exp"
iter_mod = 1
...
model = HierarchicalPredictiveCoding(dkey,
                 ...
                 r1_prior=(r1_prior_type, 1),
                 r2_prior=(r2_prior_type, 0.05),
                 U_prior = ("ridge", 0.02),
                 batch_size=mb_size,
                 exp_dir = exp_dir,
                 reset_exp_dir = False
...
model.viz_receptive_fields(patch_shape, fname='erf_t0')
################################################################################
...
for i in range(n_iter):
    ...
    if (i+1) % iter_mod == 0:
        ...
        model.viz_receptive_fields(patch_shape, stride_shape=step_shape, fname=f"erf_t{i+1}")
