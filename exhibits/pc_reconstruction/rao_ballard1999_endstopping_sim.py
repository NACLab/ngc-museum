...
"""
################################################################################
Hierarchical Predictive Coding for Endstopping Phenomena Exhibit File:

This mode is fit to learn small localized center-surround blob detectors in level 1 and
larger receptive fields spanning expanded area for extended structures in level 2
by linearly combining level 1 features from the natural_scenes database. 

Usage:
$ python rao_ballard1999_endstopping_sim.py" 
...
area_shape = (16, 26)                          ## higher level receptive field shape
patch_shape = (16, 16)                         ## level-1 receptive fields shape
step_shape = (0, 5)                            ## level-1 receptive fields distance

# ──────────────── Input Layer - Receptive Fields
n_inPatch = 3                                  ## Number of level-1 PE-modules
pin_size = patch_shape[0] * patch_shape[1]     ## Input patch dimension (256) = px * py = 16 * 16
...
p2_size = 128                                  ## Size of each level-2 PE-module
