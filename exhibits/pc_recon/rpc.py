...
class PCRecon():
    """
    Structure for constructing the predictive coding model for reconstruction task.

    Note this model imposes a laplacian prior to induce sparsity in the latent activities
    z1, z2, z3 (the latent codebook). Synapses initialized from a (He initialization) gaussian
    ...
    def __init__(self, dkey, h3_dim, h2_dim, h1_dim, io_dim, T=30, dt=1., batch_size=1,
        ...
        if load_dir is not None:
            ...
            with Context("Circuit") as self.circuit:
                ...
                # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                advance_cmd, advance_args = self.circuit.compile_by_key(self.E1, self.E2, self.E3, ## execute feedback first
                ...
                evolve_cmd, evolve_args = self.circuit.compile_by_key(self.W1, self.W2, self.W3,
                                                                      compile_key="evolve", name='evolve')
                reset_cmd, reset_args = self.circuit.compile_by_key(self.z3, self.z2, self.z1,
    ...
    def dynamic(self):  ## create dynamic commands for circuit
        W3, W2, W1, E3, E2, E1, e2, e1, e0, z3, z2, z1 = self.circuit.get_components("W3", "W2", "W1",
                                                                                    "E3", "E2", "E1",
                                                                                    "e2", "e1", "e0",
                                                                                    "z3", "z2", "z1")
    ...
    def get_synapse_stats(self, W_id='W1'):
        ...
        Args:
            W_id: the name of the chosen W (synapse)
            options: "W1" or "W2" or "W3"
