...
from data.mnist.Classes_Decomposed_Normalized_0to1.data_read_split import data_split
import os
from ngclearn import Context, numpy as jnp
from rpc import PCRecon as Model
...
Learns the representation of the input and reconstruction of the data accordingly
from the MNIST database. 

Usage:
$ python train_rpc.py --path_data="/path/to/train_patterns.npy" --n_samples=-1 --n_iter=200
...
h3_dim, h2_dim, h1_dim = (100, 100, 100)
n_samples_test = -1
...
x_train = jnp.load(path_train)[:n_samples, :]
...
x_test = jnp.load(path_test)[:n_samples, :]
...
for i in range(n_iter):
    ...
    for nb in range(n_batches):
        ...
        print("\r > Seen {} patterns   | n_batch {}/{}   | Train-Recon-Loss = {}".format(
            n_pat_seen, (nb+1), n_batches+1, L/(n_pat_seen+1)), end="")
    ...
    if i % iter_mod == 0 and i > 0:
        ...
        Xmu_test, L_test = model.process(X_test, adapt_synapses=False)
