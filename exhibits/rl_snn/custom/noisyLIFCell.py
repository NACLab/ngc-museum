...
def _dfv_internal(j, v, rfr, tau_m, refract_T, v_rest, v_decay=1.): ## raw voltage dynamics
    ...
    dv_dt = (v_rest - v) * v_decay * (1./tau_m) + (j * mask)
    #dv_dt = dv_dt * (1./tau_m)
    return dv_dt
...
class NoisyLIFCell(JaxComponent): ## leaky integrate-and-fire cell
    """
    A spiking cell based on noisy leaky integrate-and-fire (LIF) neuronal dynamics. Note that this is a modified LIF 
    motivated by the model proposed in:
    
    | Chevtchenko, Sérgio F., and Teresa B. Ludermir. "Learning from sparse and delayed rewards with a multilayer 
    ...
    | tau_m * dv/dt = (v_rest - v) + [j * R + eps], where eps ~ N(0, eps_scale)
    ...
    Args:
        ...
        enforce_wta: if True, a single-spike constraint will be enforced for
            every time step of neuronal dynamics simulated, i.e., at most, only 
            one neuron with highest potential after crossing threshold will fire/emit a pulse (Default: False)
        ...
        eps_scale: degree of noise to apply to injected/input electrical current (scale of zero-mean Gaussian noise)
    """ ## batch_size arg?

    #@deprecate_args(thr_jitter=None)
    def __init__(
            self, name, n_units, tau_m, resist_m=1., thr=-52., v_rest=-65., v_reset=-60., v_decay=1., tau_theta=1e7,
            theta_plus=0.05, refract_time=5., enforce_wta=False, integration_type="euler", surrogate_type="straight_through",
            v_min=None, eps_scale=0.2, **kwargs
    ):
        ...
        self.enforce_wta = enforce_wta ## True => constrains system to simulate 1 spike per time step
        self.v_min = v_min ## True ==> ensures voltage is never < v_rest
    ...
    def advance_state(self, dt, t):
        ...
        ## update voltage / membrane potential
        v_params = (j, self.rfr.get(), self.tau_m, self.refract_T, self.v_rest, self.v_decay)
        if self.intgFlag == 1:
        ...
        wta_s = 1.
        if self.enforce_wta: ## we modify this spike to be only a WTA spike
            wta_s = nn.one_hot(jnp.argmax(_v), num_classes=s.shape[1])
            #s = wta_s * s ## TODO: or use raw_s to hyper-polarize cells?
        ...
        if self.tau_theta > 0.:
            ...
            theta_decay = jnp.exp(-dt/self.tau_theta)
            thr_theta = self.thr_theta * theta_decay + raw_spikes * self.theta_plus
            self.thr_theta.set(thr_theta)

        ## update time-of-last spike variable(s)
        self.tols.set((1. - s) * self.tols.get() + (s * t))
        # if self.v_min is not None: ## ensure voltage never < v_rest
        #     v = jnp.maximum(v, self.v_min)

        self.v.set(v)
        self.s.set(s)
        self.s_raw.set(raw_spikes)
        self.rfr.set(rfr)
    ...
    def help(cls): ## component help function
        properties = {
            "cell_type": "NoisyLIFCell - evolves neurons according to noisy, leaky integrate-and-fire spiking dynamics."
        }
        compartment_props = {
            "inputs":
